...
//                 taille du fruit                   //
    ...
    */
    ...
    do{
        ...
        printf("taille du fruit entree incorrect, veuillez reessayer:\n");
        }
...
void panier() {
    ...
    Fich = fopen("Panier.txt", "a");
    if (Fich == NULL) {
        printf("Le fichier Panier.txt ne peut pas être ouvert.\n");
        fclose(F);
        return;
    }
    ...
    while (fgets(ligne, sizeof(ligne), F) != NULL) {
        char nom[50];
        float prix;
        sscanf(ligne, "%[^;];%*[^;];%f", nom, &prix);
        if (strcmp(nom, fruitSaisi) == 0) {
            fprintf(Fich, "Nom : %s\n", nom);
            fprintf(Fich, "Prix : %.2f\n", prix);
    ...
    if (!found) {
        printf("Le fruit '%s' n'a pas été trouvé dans le fichier StockFruits.txt.\n", fruitSaisi);
    } else {
        printf("Les informations ont été ajoutées au fichier Panier.txt.\n");
    }

    // Affichage du contenu du fichier Panier.txt
    Fich = fopen("Panier.txt", "r");
    if (Fich == NULL) {
        printf("Le fichier Panier.txt ne peut pas être ouvert pour l'affichage.\n");
        return;
    }

    printf("\nContenu du fichier Panier.txt :\n");
    printf("-----------------------------\n");
